...
def sum_wavefronts(f_in,to_intensity=False,load=True):
    ...
    f_out = h5py.File(f_in+'-summed'+'.h5', 'w')
    f_out.create_dataset('data',data=summed)
    f_out.close()

    print('data saved with dimensions',np.shape(summed))
...
def load_full(f_in,summed=True,start=100,end=150):
    ...
    intensities = np.abs(data[:,1,0,:,:])**2
    print('shape is',np.shape(intensities))
...
def full_mean(f_in,summed=True,start=100,end=150):
    ...
    if "fits" in f_in:
        ...
        intensities = load_full(f_in,summed,start,end)
...
def full_spatial_cov(f_in,summed=True,start=100,end=150):
    ...
    intensities = load_full(f_in,summed,start,end)
    nx,ny=[end-start,end-start]
    intensities = intensities[:,start:end,start:end]
    n_screens = np.shape(intensities)[0]
    #print(n_screens,nx,ny)
    mean = np.mean(intensities,axis=0)
    mean_subtracted = np.reshape(intensities-mean[np.newaxis,:,:], (n_screens, ny*nx))
    cov = np.dot(mean_subtracted.T, mean_subtracted)/(n_screens-1)

    stdev = np.sqrt(np.diagonal(cov))
    norm = np.outer(stdev,stdev)
    corr = cov/norm
    print('shape of cov is',np.shape(corr))
...
def full_spacetime_cov(f_in,spatial_cov,lag,summed=True,start=100,end=150):
    ...
    cov = np.dot(mean_subtracted[0:n_screens-lag,:].T, mean_subtracted[lag:,:])/(n_screens-1)
    ...
    print('shape of spacetime cov is',np.shape(corr))
