...
##NOTE: not for large files! we're doing this the quick and dirty way loading things in full
...
cov_spatial = stats.full_spatial_cov(filename,start=start,end=end)
print('spatial cov computed')
...
for lag in lags:
	...
	if lag==0:
		...
		cov_st = stats.full_spacetime_cov(filename,cov_spatial,lag,start=start,end=end)
...
hdul.writeto(savedir+prefix+'-covariances-iterative.fits',overwrite=True)


print('file saved at',savedir+prefix+'-covariances-iterative.fits')
